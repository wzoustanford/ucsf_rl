...
IMPROVED VERSION: Uses full sequence length without burn-in
"""
...
def evaluate_lstm_model(alpha=0.001, vp2_bins=5, sequence_length=5, burn_in_length=0, stride=1):
    ...
    IMPROVED: 
    - burn_in_length is now 0 by default, using full sequence for inference
    - stride parameter controls overlap (default=1 for maximum overlap)
    - Ensures the last timestep with mortality signal is always captured
    """
    ...
    model_path = f'experiment/lstm_block_discrete_cql_alpha{alpha_str}_bins{vp2_bins}_hdim32_seql5_best.pt'
    ...
    max_patients = len(patient_ids)  # Evaluate on all test patients
    ...
    for i, patient_id in enumerate(patient_ids[:max_patients]):
        ...
        # IMPROVED: Process patient with overlapping sequences (stride parameter)
        # This ensures we capture all timesteps including the crucial last one with mortality signal
        num_sequences = 0
        ...
        for seq_start in range(0, patient_length - sequence_length + 1, stride):
            ...
            # Track if this sequence covers the last timestep
            ...
            # IMPROVED: No burn-in split, use entire sequence for evaluation
            eval_states = states_tensor  # Use full sequence
            ...
            with torch.no_grad():
                ...
                # IMPROVED: No burn-in phase, directly get Q-values for entire sequence
                ...
                # IMPROVED: Compare with clinician actions for entire sequence
                eval_actions = seq_actions  # Use full sequence actions
        ...
        if patient_length >= sequence_length:
            assert last_timestep_covered, f"Patient {patient_id}: Last timestep not covered! Length={patient_length}, sequences={num_sequences}, stride={stride}"
...
def generate_latex_table():
    ...
    print(" IMPROVED VERSION: Using full sequence (no burn-in) + overlapping sequences")
    print("="*70)
    
    # Read model list from file
    with open('experiment/lstm_32dim_5len.md', 'r') as f: #lstm_eval_200epochs_list.md #lstm_eval_200epochs_list.md
    ...
    for alpha, bins in model_configs:
        ...
        print("  Using full sequence for inference (no burn-in)")
        print("  Using overlapping sequences with stride=1 for complete coverage")
        
        # IMPROVED: burn_in_length=0 (no burn-in), stride=1 (maximum overlap)
        lstm_results = evaluate_lstm_model(alpha=alpha, vp2_bins=bins, burn_in_length=0, stride=1)
...
\end{table}"""
    ...
    # Save to file with new name indicating improved version
    output_filename = 'table1_lstm_v2_improvedv2_fullseq_hdim32_seql20_best_deltaq_cql_evaluation_best.tex'
    with open(output_filename, 'w') as f:
        f.write(latex)
    
    print(f"\n✓ LaTeX table saved to: {output_filename}")
...
if __name__ == "__main__":
    ...
    print("\n✅ Table 1 generation complete (IMPROVED VERSION)!")