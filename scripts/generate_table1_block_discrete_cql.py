...
def evaluate_model(model_type='binary', alpha=0.001, apply_persistence=False, vp2_bins=None):
    ...
    for patient_id, (start_idx, end_idx) in patient_groups.items():
        ...
        for t in range(len(patient_states)):
            ...
            clinician_q_values.append(q_clinician)
...
def generate_latex_table():
    ...
    print(f"  Binary CQL...")
    binary_results = evaluate_model('binary', 0.001, apply_persistence=True)
    
    # Dual Mixed CQL
    print(f"  Dual Mixed CQL...")
    dual_results = evaluate_model('dual', 0.010, apply_persistence=True)
    """
    results[alpha] = {
    ...
    print('exiting from generate latex table for generating results')
    exit()
    # Generate LaTeX
