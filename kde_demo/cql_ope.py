"""
Off-Policy Evaluation for CQL Models
=====================================

Generic OPE framework for CQL models using KDE-based importance sampling.
Currently supports binary CQL models, with extensibility for other variants.
"""
...
class CQLAgent:
    ...
    def get_q_values(self, states: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        """Get Q-values using double Q-learning"""
        with torch.no_grad():
...
class BinaryCQLAgent(CQLAgent):
    ...
    def select_action(self, state: np.ndarray) -> float:
        ...
        with torch.no_grad():
            ...
            q0 = self.get_q_values(state_tensor, action_0).item()
            q1 = self.get_q_values(state_tensor, action_1).item()
    ...
    def select_actions_batch(self, states: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # Evaluate both actions for all states
            action_0 = torch.zeros(batch_size, 1).to(self.device)
            action_1 = torch.ones(batch_size, 1).to(self.device)
            
            q0 = self.get_q_values(states_tensor, action_0).squeeze()
            q1 = self.get_q_values(states_tensor, action_1).squeeze()
            
            # Select best actions
            actions = (q1 > q0).float().cpu().numpy()
