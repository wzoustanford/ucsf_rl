#!/usr/bin/env python3
"""
Unified training script for Binary and Dual CQL with alpha=0.01
...
def train_binary_cql():
    """Train Binary CQL with alpha=0.01"""
    print("="*70, flush=True)
    print(" BINARY CQL TRAINING WITH ALPHA=0.01", flush=True)
    print("="*70, flush=True)
    ...
    print("  ALPHA = 0.01 (minimal conservative penalty)", flush=True)
    ...
    agent = BinaryCQL(
        state_dim=state_dim,
        alpha=0.01,   # Minimal CQL penalty
        gamma=0.95,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            agent.save('experiment/binary_cql_unified_alpha01_best.pt')
    ...
    # Save final model
    agent.save('experiment/binary_cql_unified_alpha01_final.pt')
    
    total_time = time.time() - start_time
    print(f"\n✅ Binary CQL training completed in {total_time/60:.1f} minutes!", flush=True)
    print("Models saved:", flush=True)
    print("  - experiment/binary_cql_unified_alpha01_best.pt", flush=True)
    print("  - experiment/binary_cql_unified_alpha01_final.pt", flush=True)
    
    return agent, pipeline


def train_dual_cql():
    """Train Dual Continuous CQL with alpha=0.01"""
    print("\n" + "="*70, flush=True)
    print(" DUAL CONTINUOUS CQL TRAINING WITH ALPHA=0.01", flush=True)
    print("="*70, flush=True)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'gamma': 0.95,
                'tau': 0.8
            }, 'experiment/dual_cql_unified_alpha01_best.pt')
    ...
    torch.save({
        ...
        'gamma': 0.95,
        'tau': 0.8
    }, 'experiment/dual_cql_unified_alpha01_final.pt')
    
    total_time = time.time() - start_time
    print(f"\n✅ Dual CQL training completed in {total_time/60:.1f} minutes!", flush=True)
    print("Models saved:", flush=True)
    print("  - experiment/dual_cql_unified_alpha01_best.pt", flush=True)
    print("  - experiment/dual_cql_unified_alpha01_final.pt", flush=True)
    
    return agent, pipeline


def main():
    """Train both Binary and Dual CQL with alpha=0.01"""
    print("="*70, flush=True)
    print(" UNIFIED CQL TRAINING - ALPHA=0.01", flush=True)
    print("="*70, flush=True)
    ...
    print("  Binary CQL: binary_cql_unified_alpha01_*.pt", flush=True)
    print("  Dual CQL:   dual_cql_unified_alpha01_*.pt", flush=True)
