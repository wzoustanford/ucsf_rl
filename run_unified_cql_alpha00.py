...
def train_dual_cql():
    """Train Dual Continuous CQL with alpha=0.0"""
    print("\n" + "="*70, flush=True)
    print(" DUAL CONTINUOUS CQL TRAINING WITH ALPHA=0.0", flush=True)
    print("="*70, flush=True)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'gamma': 0.95,
                'tau': 0.8
            }, 'experiment/dual_cql_unified_alpha00_best.pt')
    ...
    torch.save({
        ...
        'gamma': 0.95,
        'tau': 0.8
    }, 'experiment/dual_cql_unified_alpha00_final.pt')
    
    total_time = time.time() - start_time
    print(f"\n✅ Dual CQL training completed in {total_time/60:.1f} minutes!", flush=True)
    print("Models saved:", flush=True)
    print("  - experiment/dual_cql_unified_alpha00_best.pt", flush=True)
    print("  - experiment/dual_cql_unified_alpha00_final.pt", flush=True)
    
    return agent, pipeline


def main():
    """Train both Binary and Dual CQL with alpha=0.0"""
    print("="*70, flush=True)
    ...
    print("  Dual CQL:   dual_cql_unified_alpha00_*.pt", flush=True)
