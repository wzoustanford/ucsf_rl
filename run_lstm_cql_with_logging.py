#!/usr/bin/env python3
"""
LSTM-based Sequential CQL Training Script with Detailed Logging
================================================================
Enhanced version with comprehensive logging of all training metrics.
"""
...
def train_lstm_binary_cql(
    ...
    """
    Train LSTM-based Binary CQL with detailed logging.
    """
    # Create directories
    ...
    log_path = os.path.join(log_dir, f'lstm_cql_alpha{alpha:.2f}_{timestamp}.log')
    logger = Logger(log_path)
    
    logger.log("="*70)
    logger.log(f" LSTM BINARY CQL TRAINING WITH ALPHA={alpha}")
    logger.log("="*70)
    ...
    config_path = os.path.join(log_dir, f'lstm_cql_config_{timestamp}.json')
    ...
    logger.log(f"  Action dimension: 1 (binary)")
    ...
    for patient_id, (start_idx, end_idx) in pipeline.train_patient_groups.items():
        for t in range(start_idx, end_idx):
            train_buffer.add_transition(
                ...
                action=np.array([train_data['actions'][t]]),
    ...
    for patient_id, (start_idx, end_idx) in pipeline.val_patient_groups.items():
        for t in range(start_idx, end_idx):
            val_buffer.add_transition(
                ...
                action=np.array([val_data['actions'][t]]),
    ...
    # Initialize LSTM CQL agent
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    logger.log(f"\nInitializing LSTM CQL agent on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['mean_q_value'] > best_val_q:
            ...
            save_path = os.path.join(save_dir, f'lstm_cql_alpha{alpha:.2f}_best.pt')
            torch.save({
    ...
    # Save final model
    final_save_path = os.path.join(save_dir, f'lstm_cql_alpha{alpha:.2f}_final.pt')
    torch.save({
        'q1_state_dict': agent.q1.state_dict(),
        'q2_state_dict': agent.q2.state_dict(),
        'q1_target_state_dict': agent.q1_target.state_dict(),
    ...
    total_time = time.time() - start_time
    logger.log("\n" + "="*70)
    logger.log(f"✅ LSTM CQL training completed in {total_time/60:.1f} minutes!")
...
def main():
    ...
    print("="*70)
    print(" LSTM SEQUENTIAL CQL TRAINING WITH LOGGING")
    print("="*70)
    print("\nTraining LSTM-based CQL with detailed logging")
    print("Log files will be saved to logs/ directory")
    print("Models will be saved to experiment/ directory")
    
    # Train with alpha=0.0 (pure Q-learning)
    agent, train_buffer, val_buffer, history = train_lstm_binary_cql(
        alpha=0.001,
        sequence_length=20,
        burn_in_length=8,
        overlap=10,
